...
public class CargaImagenService : ICargaImagenes
{
    ...
    */
    ...
    private static string ObtenerRutaCompletaExpedientes(string cadena)
    {
        cadena = Path.GetFileNameWithoutExtension(cadena);
        /*
        if (cadena.Length < 13)
        ...
        */
        ...
        string segundoValor = cadena[..3].PadRight(3, '0');
        string tercerValor = cadena[..14].PadRight(14, '0');
        string ruta = $@"\UE\{primerValor}\{segundoValor}\{tercerValor}\{cadena}.PDF";

        return ruta;
    }
    private static string ObtenerRutaCompletaSinArchivoExpedientes(string cadena)
    {
        cadena = Path.GetFileNameWithoutExtension(cadena);
        if (cadena.Length < 14)
            throw new ArgumentException("La cadena debe tener al menos 15 caracteres.");
        ...
        string segundoValor = cadena[..3].PadRight(3, '0');
        string tercerValor = cadena[..14].PadRight(14, '0');
        string ruta = $@"\UE\{primerValor}\{segundoValor}\{tercerValor}";
    ...
    public async Task<IEnumerable<ArchivoImagenCorta>> CargaImagenesExpedientesAsync(string directorioOrigen, IProgress<ReporteProgresoProceso> progreso, string unidadTemporal = "")
    {
        ...
        try
        {
            ...
            foreach (var nombreArchivo in listaArchivos)
            {
                ...
                string numeroContrato = Condiciones.ObtieneNumContrato(Condiciones.LimpiaNoNumeros(Path.GetFileName(nombreArchivo)));
                string soloPDF = Path.GetFileName(nombreArchivo);
                string soloExtension = new FileInfo(ObtenerRutaCompletaExpedientes(numeroCredito).Trim()).Extension;
                string carpetaDestino = driveImagenes + "11\\UE" + (Path.GetDirectoryName(nombreArchivo) ?? "").Replace("z:","", StringComparison.InvariantCultureIgnoreCase);
                // Path.GetDirectoryName(nombreArchivo).Replace("z:", "E:\\11\\UE", StringComparison.InvariantCultureIgnoreCase)
                if (numeroCredito.Equals("000000000000000000"))
                {
                    // Ya no lo descargo
                    continue;
                ...
                listaImagenesAProcesar.Add(new()
                {
                    ...
                    FechaDeModificacion = DateTime.Now,
                    NombreArchivo = soloPDF,
                    RutaDeGuardado = ObtenerRutaCompletaSinArchivoExpedientes(numeroCredito),
                    UsuarioDeModificacion = "",
                    NumCredito = numeroCredito,
                    NumContrato = numeroContrato,
                    NumeroCreditoActivo = numeroCredito,
    ...
    /// </summary>
    /// <param name="progreso">Barra de progreso</param>
    /// <returns></returns>
    public async Task LimpiaImagenes(IProgress<ReporteProgresoProceso> progreso)
    {


        int cantidadDePasos = 11;
        int etapa = 1;
        IEnumerable<ArchivosImagenes> imagenesPorcesadas;
        IList<ArchivosImagenes> procesadas = new List<ArchivosImagenes>();
        IList<ArchivosImagenes> duplicadas = new List<ArchivosImagenes>();
        IList<ArchivosImagenes> noLocalizadas = new List<ArchivosImagenes>();
        IList<ArchivosImagenes> noLocalizadasConHash = new List<ArchivosImagenes>();

        #region Corrige error del bat
        var noLocalizadas2 = await _servicioImagenes.CargaImagenesTratadasAsync(@"C:\202302 Digitalizacion\6. Resultado de las imagenes\Nolocalizadas.xlsx");
        if (noLocalizadas2 != null)
        {
            StringBuilder sbx = new();
            sbx.AppendLine("cd \\");
            foreach (var img in noLocalizadas2)
            {

                string carpetaOrigen = img.CarpetaDestino ?? "";
                carpetaOrigen = CambiarUnidad(carpetaOrigen, "G:\\");
                string carpetaDestino = img.CarpetaDestino ?? "";
                carpetaDestino = CambiarUnidad(carpetaDestino, "E:\\");

                string fileName = Path.GetFileName(img.NombreArchivo ?? "");
                string fullPathDestino = Path.Combine(carpetaDestino, fileName);
                string fullPathOrigen = Path.Combine(carpetaOrigen, fileName);


                sbx.AppendLine(string.Format("xcopy \"{0}\" \"{1}\" /s/e/v/d/y", fullPathOrigen, fullPathDestino));
                Console.WriteLine(img.NombreArchivo);
            }
            File.WriteAllText(@"C:\202302 Digitalizacion\6. Resultado de las imagenes\copiaArchivosFaltantes2.bat", sbx.ToString());
            return;
        }
        #endregion
        ...
        etapa++;
        progreso.Report(new ReporteProgresoProceso { NumeroPasoActual = etapa, CantidadDePasos = cantidadDePasos, EtiquetaDePaso = "Cargando las imagenes de expedientes", NumeroProgresoSubProceso = 2, TotalProgresoSubProceso = 3, EtiquetaSubProceso = "Cargando el registro de todas las imagenes registradas" });
        imagenesPorcesadas = await _servicioImagenes.CargaImagenesTratadasAsync();
        etapa++;
        long archivosProcesados = 0;
        long archivosTotales = imagenesPorcesadas.Count();
        await Task.Run(() => {
            foreach (var img in imagenesPorcesadas)
            {
                archivosProcesados++;
                progreso.Report(new ReporteProgresoProceso
                {
                    ...
                    EtiquetaDePaso = "Limpiando imagenes",
                    NumeroProgresoSubProceso = 1,
                    TotalProgresoSubProceso = 1,
                    EtiquetaSubProceso = "Revisando imagen por imagen",
                    ArchivoProcesado = archivosProcesados,
                    CantidadArchivos = archivosTotales,
                    InformacionArchivo = img.NombreArchivo
                });
                string carpetaDestino = img.CarpetaDestino ?? "";
                carpetaDestino = CambiarUnidad(carpetaDestino, driveImagenes);
                string fileName = Path.GetFileName(img.NombreArchivo ?? "");
                string fullPath = Path.Combine(carpetaDestino, fileName);
                if (!File.Exists(fullPath))
                {
                    noLocalizadas.Add(img);
                    if (!string.IsNullOrEmpty(img.Hash))
                    {
                        noLocalizadasConHash.Add(img);
                    }
                }
                else
                {
                    bool duplicado = false;
                    if (img.NumCredito == "000000000000000000")
                    {
                        img.NumCte = "";
                        img.Acreditado = "";
                        img.NumContrato = "";
                        if (!string.IsNullOrEmpty(img.OcrNumCliente) && !((img.OcrNumCredito ?? "").Equals("000000000000000000")))
                        {
                            img.NumCredito = img.OcrNumCredito;
                            img.NumCte = img.OcrNumCliente;
                        }
                        else
                        {
                            if (!string.IsNullOrEmpty(img.Hash))
                            {
                                var buscaHash = imagenesPorcesadas.Where(x => (x.Id != img.Id) && (x.Hash ?? "").Equals(img.Hash)).FirstOrDefault();
                                if (buscaHash != null)
                                {
                                    duplicado = true;
                                    img.NumCredito = buscaHash.NumCredito;
                                    img.NumCte = buscaHash.NumCte;
                                }
                            }
                        }
                    }
                    ...
                    {
                        var buscaHash = imagenesPorcesadas.Where(x => (x.Id != img.Id) && (x.Hash ?? "").Equals(img.Hash)).FirstOrDefault();
                        duplicado = (buscaHash != null);
                    }
                    if (duplicado)
                    {
                        var buscaDuplicadoProcesado = procesadas.Where(x => (x.Hash ?? "").Equals(img.Hash) && (x.NumCredito??"").Equals(img.NumCredito)).FirstOrDefault();
                        // Si no existe en procesadas, entonces aun no es duplicada
                        if (buscaDuplicadoProcesado == null)
                        { 
                            procesadas.Add(img);
                        }
                        // Si ya existe en procesadas, entonces es duplicada
                        else
                        {
                            duplicadas.Add(img);
                        }
        ...
        await _servicioImagenes.GuardarImagenesAsync(noLocalizadas, @"C:\202302 Digitalizacion\6. Resultado de las imagenes\Nolocalizadas.xlsx");
        Console.WriteLine("No localizadas: " + noLocalizadas.Count());
        await _servicioImagenes.GuardarImagenesAsync(procesadas, @"C:\202302 Digitalizacion\6. Resultado de las imagenes\procesadas.xlsx");
        Console.WriteLine("Procesadas: " + procesadas.Count());
        await _servicioImagenes.GuardarImagenesAsync(duplicadas, @"C:\202302 Digitalizacion\6. Resultado de las imagenes\duplicadas.xlsx");
        Console.WriteLine("Duplicadas: " + duplicadas.Count());
        StringBuilder sb = new();
        sb.AppendLine("cd \\");
        foreach (var img in duplicadas)
        {
            ...
            sb.AppendLine(string.Format("xcopy \"{0}\" \"{1}\" /s/e/v/d/y",fullPathOrigen, fullPathDestino));
            Console.WriteLine(img.NombreArchivo);
        }
        File.WriteAllText(@"C:\202302 Digitalizacion\6. Resultado de las imagenes\copiaArchivosFaltantes.bat", sb.ToString());
    }

    private static string CambiarUnidad(string path, string nuevaUnidad)
    {
        ...
        string unidadOriginal = path.Substring(0, 2); // Obtener la unidad original
        string pathRestante = path.Substring(3); // Obtener el resto de la ruta después de la unidad

        return nuevaUnidad + pathRestante; // Concatenar la nueva unidad con el resto de la ruta
    }
}
