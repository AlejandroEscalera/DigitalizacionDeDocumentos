...
#pragma warning disable IDE0063 // Use la instrucción "using" simple
            using (FileStream fs = new(archivoExcelOrigen, FileMode.Open, FileAccess.Read))
            {
...
#pragma warning restore IDE0063 // Use la instrucción "using" simple
        ...
        public bool GuardarImagenes(IEnumerable<ArchivosImagenes> imagenes, string archivoSalida = "")
        {
            ...
            OfficeOpenXml.ExcelPackage.LicenseContext = OfficeOpenXml.LicenseContext.NonCommercial;
#pragma warning disable IDE0063 // Use la instrucción "using" simple
            using (FileStream fs = new(archivoResultadoImagenes ?? "", FileMode.CreateNew, FileAccess.ReadWrite, FileShare.ReadWrite))
            {
                using (var package = new ExcelPackage(fs))
                {
                    ...
                    package.Save();
                    // Asegurar que se complete el guardado
...
#pragma warning restore IDE0063 // Use la instrucción "using" simple
        ...
        public async Task<IEnumerable<ArchivosImagenes>> CargaImagenesTratadasAsync(string archivo = "")
        {
            if (string.IsNullOrEmpty(archivo))
                archivo = _archivoResultadoImagenes;
            OfficeOpenXml.ExcelPackage.LicenseContext = OfficeOpenXml.LicenseContext.NonCommercial;
...
#pragma warning disable IDE0063 // Use la instrucción "using" simple
            using (FileStream fs = new(archivo, FileMode.Open, FileAccess.Read, FileShare.Read))
            {
                using (var package = new ExcelPackage())
                {
                    ...
                    int numHojas = await Task.Run(() => { return package.Workbook.Worksheets.Count; });
                    if (numHojas == 0)
                        return resultado;
                    var hoja = package.Workbook.Worksheets[0];
                    ObtieneValoresCampos(hoja);
...
#pragma warning restore IDE0063 // Use la instrucción "using" simple
        ...
        public async Task<bool> GuardarImagenesAsync(IEnumerable<ArchivosImagenes> imagenes, string archivoSalida = "")
        {
            string archivoResultadoImagenes;
            if (string.IsNullOrEmpty(archivoSalida))
                archivoResultadoImagenes = _archivoResultadoImagenes;
